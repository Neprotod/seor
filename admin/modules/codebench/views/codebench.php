...
<head>
    ...
    <script>
        $(document).ready(function() {
            ...
            <?php if (Admin_Module::config('codebench','codebench')->expand_all) { ?>
...
<body>
    ...
    <form id="runner" method="post" action="<?php echo URL::root(FALSE) ?>">
        <input name="session_id" value="<?php echo Session::i()->id() ?>" type="hidden">
        <h1>
    ...
    <?php if ( ! empty($codebench)) { ?>
                    ...
                    <div>
                        <table>
                            ...
                            <?php foreach ($benchmark['subjects'] as $subject_key => $subject) { ?>
                                <tr>
                                    <td>
                                        <strong class="help" title="(<?php echo gettype($codebench['subjects'][$subject_key]) ?>) <?php echo Str::html_encode(var_export($codebench['subjects'][$subject_key], TRUE)) ?>">
                                            [<?php echo Str::html_encode($subject_key) ?>] →
                                        </strong>
                                        <span class="quiet">(<?php echo gettype($subject['return']) ?>)</span>
                                        <?php echo Str::html_encode(var_export($subject['return'], TRUE)) ?>
                                    </td>
                                    <td class="numeric">
        ...
        <?php // echo '<h2>Raw output:</h2>', Core::debug($codebench) ?>
