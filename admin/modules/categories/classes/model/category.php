 ...
 */
class Model_Category_Categories_Admin{
    ...
    function get($id){
        ...
        $category = $this->method->get_category($id,$template['id']);
        
        //Обработка пост запроса
        $this->post($category,$template);
        
        
        if(!empty($category['robots'])){
            $check_robots = array_flip(explode(',',$category['robots']));
        ...
        Registry::i()->title = "Категория: ".$category['title'];
        ...
        $date['style'] = $this->style($template,$category);
        $date['type'] = $this->type($template,$category);
    ...
    function post(&$category,$template){
    ...
    protected function style($template,$category){
        ...
        //Стили категории
        $style = $this->style->get_styles($template['name'],array('category',NULL));
        
        //Стили для расширения
        $extends = $this->style->get_styles($template['name'],array('post',NULL));
        ...
        //Добавляем статус для используемых стилей расширения
        $exists = array_intersect_key($style_contents,$extends);
        if(!empty($exists))
            foreach($exists AS $key => $exist)
                $extends[$key]['status'] = $exist['status'];
