...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:template match="alert">
        ...
        <div class="alert_box">
            ...
            <xsl:if test="title != '' or massage != ''">
                <div class="alert {$type}">
                    <button type="button" data-dismiss="alert" class="close hidden"><span>x</span></button>
                    <div>
                        <strong><xsl:value-of select="title" /></strong> <xsl:value-of disable-output-escaping="yes" select="concat(' ',massage)" />
                    </div>
                </div>
            </xsl:if>
        </div>
