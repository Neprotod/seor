...
<div class="extends">
    <div class="styles">
        <h3>Стили которые должен наследовать пост</h3>
        <?php
        foreach($extends AS $key => $values):
            $title = '';
            switch($key){
                case 'default':
                    $title = 'По умолчанию';
        ...
        ?>
        <div class="box <?=$key?>">
            <h3>Стили: <?=(!empty($title))? $title : $key ; ?></h3>
            <?php
            foreach($values AS $style_type => $value):
            ?>    
            <div class="<?=$style_type?>">
                ...
                <label class="style <?=$style['type']?>">
                    <div class="action">
                        <input type="checkbox" <?=((isset($style['default']) AND $style['default'] == 1 AND !isset($style['status'])) OR (isset($style['status']) AND $style['status'] == 1) )? 'checked="checked"' : ''?> value="1" name="extends_style[<?=$id?>]" style="">
                    </div>
                    <div class="information">
                        <span class="type"><?=$type?></span>
                        <span class="name"><?=$style['name']?></span>
