...
class Core_Database_Database_Query {
    ...
    /**
     * Добавление нескольких параметров в запрос.
     *
     ...
     *
     * @param   string   По какому полю сортировать
     * @param   boolean  Если в TRUE вернет один результат.
     * @param   mixed    Экземпляр базы данных или имя экземпляра
     * @return  object   Database_Result for SELECT queries
     * @return  mixed    the insert id for INSERT queries
     * @return  integer  number of affected rows for all other queries
     */
    public function execute($desc = NULL, $one = NULL, $db = NULL){
        if (!is_object($db)){
        ...
        if(!empty($desc) AND !empty($result)){
            if(array_key_exists($desc,reset($result))){
                $fonds = array();
                foreach($result as $key => $value){
                    $fonds[$result[$key][$desc]] = $value;
                }
                $result = Request::unsets($fonds);
            }else{
                throw new Core_Exception("В таблице нет значения <b>:desc</b> для сортировки",array(':desc'=>$desc));
            }
        }
        
        //Если одна запись
        if($one === TRUE AND !empty($result)){
            $result = reset($result);
        }
