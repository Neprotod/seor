 ...
 * @category   page
 */
class Controller_User_Account_User{
    ...
    function fetch(){
        ...
        if($user["complete"] == 0 OR Request::get("edit")){
            ...
            $xsl_pars = "template|default::user_user_account";
            ...
            $data["user_language"] = Query::i()->sql("user.language",array(
                                                                ":id" => $user["id"] 
                                                            ),"id_language");
                                                            
            $data["user_specialization"] = Query::i()->sql("user.specialization",array(
                                                                ":id" => $user["id"] 
                                                            ),"id_specialization");
            // Создаем логотипы
    ...
    function edit(){
        ...
        // Парсер файла
        $xml_pars = "template|default::user_account_profile";
        $xsl_pars = "template|default::user_account_profile_user";
        ...
        if(Request::method("post")){
            ...
            try{
                ...
                if(!$this->account->user_lang_spec_post($data)){
                    $flag = FALSE;
                }
